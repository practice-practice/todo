...
describe('Todo endpoints', function() {
  ...
  before(function(done) {
    ...
    newTodo.message = 'Must finish project!';
  ...
  it('should get a todo', function(done) {
    chai.request(urlBase)
      ...
      .end(function(err, res) {
        ...
        expect(res.body[0].message).to.eql('Must finish project!');
  ...
  it('should create a todo', function(done) {
    chai.request(urlBase)
      .post('/todo')
      .send({'message': 'Build an app.'})
      .end(function(err, res) {
        expect(err).to.eql(null);
        expect(res.body).to.exist; // jshint ignore:line
        expect(res.body.message).to.eql('Build an app.');
  ...
  it('should overwrite a todo', function(done) {
    chai.request(urlBase)
      .put('/todo/' + todoId)
      .send({'message': 'Build a better app!'})
      .end(function(err, res) {
        expect(err).to.eql(null);
        expect(res.body).to.exist; // jshint ignore:line
        expect(res.body.message).to.eql('Build a better app!');
